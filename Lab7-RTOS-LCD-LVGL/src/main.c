...
void lv_termostato(void) {
	...
	lv_obj_align_to(labelSetValue, labelClock, LV_ALIGN_OUT_BOTTOM_MID, 0 , 20);
	...
	lv_obj_set_width(btn1, 50);  lv_obj_set_height(btn1, 60);
	
	lv_obj_t * btn2 = lv_btn_create(lv_scr_act());
	lv_obj_add_event_cb(btn2, event_handler2, LV_EVENT_ALL, NULL);
	lv_obj_set_width(btn2, 50);  lv_obj_set_height(btn2, 60);
	...
	lv_obj_set_width(btn3, 50);  lv_obj_set_height(btn3, 60);
	...
	lv_obj_align(btn4, LV_ALIGN_BOTTOM_RIGHT, 0, -10);
	lv_obj_add_style(btn4, &style, 0);
	lv_obj_set_width(btn4, 50);  lv_obj_set_height(btn4, 60);
	
	lv_obj_t * btn5 = lv_btn_create(lv_scr_act());
	lv_obj_add_event_cb(btn5, up_handler, LV_EVENT_ALL, NULL);
	lv_obj_set_width(btn5, 50);  lv_obj_set_height(btn5, 60);
...
static void task_tempo(void *pvParameters){
	...
	volatile int flag_ponto = 0;
	
	for(;;){
		if(xSemaphoreTake(xSemaphoreRTC, 500)){
			rtc_get_time(RTC, &current_hour, &current_min, &current_sec);
			if(flag_ponto == 0){
				lv_label_set_text_fmt(labelClock, "%02d:%02d", current_hour,current_min);
				flag_ponto = 1;
			} else if(flag_ponto == 1){
				lv_label_set_text_fmt(labelClock, "%02d %02d", current_hour,current_min);
				flag_ponto = 0;
			}
		}
...
int main(void) {
	...
	if (xTaskCreate(task_tempo, "tempo", TASK_LCD_STACK_SIZE, NULL, TASK_LCD_STACK_PRIORITY, NULL) != pdPASS) {
