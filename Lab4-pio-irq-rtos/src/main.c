...
QueueHandle_t xQueueAumentaLed;
...
int main(void) {
	...
	if (xSemaphoreBut2 == NULL){
		printf("falha em criar o segundo semaforo \n");
	}
	
	/* cria queue com 32 "espacos" */
	/* cada espaço possui o tamanho de um inteiro*/
	xQueueLedFreq = xQueueCreate(32, sizeof(uint32_t));
	xQueueAumentaLed = xQueueCreate(32,sizeof(uint32_t));
	if (xQueueLedFreq == NULL){
		printf("falha em criar a primeira queue \n");
	}
	if (xQueueAumentaLed == NULL){
		printf("falha em criar a segunda queue \n");
	}
